...
void piface_putc(char c){
    /* Handle newline character first */
    if (c == '\n') {
		if (cnt%32 == 0) {
			lcd_write_cmd(0x02);  // Move to beginning of first line
			cnt = 0;
			return;  // Don't write the newline character to LCD
		}
		lcd_write_cmd((0x40|0x80));  // Move to second line
		cnt = 16;
        return;  // Don't write the newline character to LCD
    }
