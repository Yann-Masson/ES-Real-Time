...
static void enqueue(thread p, thread *queue)
{
	...
	if (*queue == NULL || (*queue)->Period_Deadline > p->Period_Deadline)
	{
		p->next = *queue;
		*queue = p;
	}
...
static void sortX(thread *queue)
{
	...
	while (current)
	{
		...
		if (sorted == NULL || current->Rel_Period_Deadline < sorted->Rel_Period_Deadline)
		{
			current->next = sorted;
			sorted = current;
		}
		else
		{
			...
			while (tmp->next && tmp->next->Rel_Period_Deadline <= current->Rel_Period_Deadline)
			{
				tmp = tmp->next;
			}
...
void respawn_periodic_tasks(void)
{
	...
	while (d && ticks >= d->Period_Deadline)
	{
